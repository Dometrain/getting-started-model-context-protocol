...
**With Azure AD (Entra ID) Authentication.** Builds on section 03 by adding JWT bearer auth - the MCP endpoint requires a valid access token with the `mcp.tools` scope.

## Before you run

1. **Azure AD (Entra ID)**: Replace `<AZURE_TENANT_ID>` and `<AZURE_CLIENT_ID>` in `src/McpServerAuthenticated/appsettings.json` and `.vscode/launch.json` (for F5 debugging) with your app registration's Tenant ID and Application (client) ID from Azure Portal.
